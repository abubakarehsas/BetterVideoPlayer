 ...
 */
public class BetterVideoPlayer extends RelativeLayout
    implements IUserMethods, TextureView.SurfaceTextureListener, MediaPlayer.OnPreparedListener,
  ...
  private static final int MATERIAL_PROGRESS = 11;
  private int mLoadingStyle;

  @IntDef({
      DOUBLE_BOUNCE, ROTATING_PLANE, WAVE, WANDERING_CUBES, PULSE, CHASING_DOTS, THREE_BOUNCE,
      CIRCLE, CUBE_GRID, FADING_CIRCLE, ROTATING_CIRCLE, MATERIAL_PROGRESS
  }) @Retention(RetentionPolicy.SOURCE) public @interface LoadingStyle {
  }
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      TypedArray a =
          context.getTheme().obtainStyledAttributes(attrs, R.styleable.BetterVideoPlayer, 0, 0);
      try {
        ...
        if (title != null && !title.trim().isEmpty()) mTitle = title;
        ...
        mHideControlsDuration = a.getInteger(R.styleable.BetterVideoPlayer_bvp_hideControlsDuration,
            mHideControlsDuration);

        mHideControlsOnPlay =
            a.getBoolean(R.styleable.BetterVideoPlayer_bvp_hideControlsOnPlay, false);
        ...
        mShowTotalDuration =
            a.getBoolean(R.styleable.BetterVideoPlayer_bvp_showTotalDuration, false);
        mShowBottomProgressBar =
            a.getBoolean(R.styleable.BetterVideoPlayer_bvp_showBottomProgressBar, false);
  ...
  @Override public void setSource(@NonNull Uri source) {
    mSource = source;
    if (mPlayer != null) prepare();
  }

  @Override public void setSource(@NonNull Uri source, @NonNull Map<String, String> headers) {
    this.headers = headers;
    setSource(source);
  }

  @Override public void setCallback(@NonNull BetterVideoCallback callback) {
    mCallback = callback;
  }
  ...
  @Override public void setProgressCallback(@NonNull BetterVideoProgressCallback callback) {
    mProgressCallback = callback;
  }

  @Override public void setTitle(@NonNull String text) {
    mTitle = text;
  ...
  @Override public void setMenu(@MenuRes int resId) {
    mMenuId = resId;
    mToolbar.inflateMenu(mMenuId);
  }

  @Override public void setMenuCallback(@NonNull Toolbar.OnMenuItemClickListener callback) {
    menuItemClickListener = callback;
    mToolbar.setOnMenuItemClickListener(menuItemClickListener);
  }

  @Override public void setPlayDrawable(@NonNull Drawable drawable) {
    mPlayDrawable = drawable;
    if (!isPlaying()) mBtnPlayPause.setImageDrawable(drawable);
  }

  @Override public void setPlayDrawable(@DrawableRes int res) {
    setPlayDrawable(ContextCompat.getDrawable(getContext(), res));
  }

  @Override public void setPauseDrawable(@NonNull Drawable drawable) {
    mPauseDrawable = drawable;
    if (isPlaying()) mBtnPlayPause.setImageDrawable(drawable);
  }

  @Override public void setPauseDrawable(@DrawableRes int res) {
    setPauseDrawable(ContextCompat.getDrawable(getContext(), res));
  }

  @Override public void setWindow(@NonNull Window window) {
    mWindow = window;
  }

  @Override public void setHideControlsOnPlay(boolean hide) {
  ...
  @Override public void setAutoPlay(boolean autoPlay) {
    mAutoPlay = autoPlay;
  }

  @Override public void setInitialPosition(@IntRange(from = 0, to = Integer.MAX_VALUE) int pos) {
  ...
  @Override public void showControls() {
    ...
    subViewParent.animate().translationY(0).setInterpolator(new DecelerateInterpolator()).start();
  ...
  @Override public void hideControls() {
    ...
    mControlsFrame.animate()
        ...
        .setListener(new AnimatorListenerAdapter() {
          @Override public void onAnimationEnd(Animator animation) {
            if (mControlsFrame != null) mControlsFrame.setVisibility(View.GONE);
          }
        })
        .start();
    ...
    mTooolbarFrame.animate()
        ...
        .setListener(new AnimatorListenerAdapter() {
          @Override public void onAnimationEnd(Animator animation) {
            if (mTooolbarFrame != null) mTooolbarFrame.setVisibility(View.GONE);
          }
        })
        .start();
  }

  @CheckResult @Override public boolean isControlsShown() {
  ...
  @Override public void enableControls(boolean andShow) {
    mControlsDisabled = false;
  ...
  @Override public void disableControls() {
  ...
  @CheckResult @Override public boolean isPrepared() {
    return mPlayer != null && mIsPrepared;
  }

  @CheckResult @Override public boolean isPlaying() {
    return mPlayer != null && mPlayer.isPlaying();
  }

  @CheckResult @Override public int getCurrentPosition() {
    if (mPlayer == null) return -1;
    return mPlayer.getCurrentPosition();
  }

  @CheckResult @Override public int getDuration() {
    if (mPlayer == null) return -1;
  ...
  @Override public void seekTo(@IntRange(from = 0, to = Integer.MAX_VALUE) int pos) {
    if (mPlayer == null) return;
    mPlayer.seekTo(pos);
  }
  ...
  @Override public void setCaptions(Uri source, CaptionsView.CMime cMime) {
    mSubView.setCaptionsSource(source, cMime);
  }

  @Override public void setCaptions(@RawRes int resId, CaptionsView.CMime cMime) {
    mSubView.setCaptionsSource(resId, cMime);
  }

  @Override public void removeCaptions() {
  ...
  @Override public void setLoadingStyle(@LoadingStyle int style) {
    Drawable drawable;
    switch (style) {
      case DOUBLE_BOUNCE:
  ...
  private Drawable materialProgress() {
    ...
    shapeD.getPaint().setColor(Color.GRAY);
    ...
    @Override public void onMove(Direction dir, float diff) {
      ...
      if (dir == Direction.LEFT || dir == Direction.RIGHT) {
        if (mPlayer.getDuration() <= 60) {
          ...
          LOG("Difftime is %f and duration %d, physical diff %f", diffTime, mPlayer.getDuration(),
              diff);
        } else {
        ...
        String progressText =
            Util.getDurationString((long) finalTime, false) + " [" + (dir == Direction.LEFT ? "-"
        ...
        if (initialX >= mInitialTextureWidth / 2 || mWindow == null) {
          ...
          Log.d("VolumeControl",
              (diff / (float) mInitialTextureHeight) + " " + diff + " " + mInitialTextureHeight);
          ...
          String progressText =
              String.format(getResources().getString(R.string.volume), finalVolume);
          mPositionTextView.setText(progressText);
          ...
          String progressText =
              String.format(getResources().getString(R.string.brightness), finalBrightness);
          mPositionTextView.setText(progressText);
  ...
  private final Runnable mUpdateCounters = new Runnable() {
    @Override public void run() {
      ...
      if (mProgressCallback != null) mProgressCallback.onVideoProgressUpdate(position, duration);
